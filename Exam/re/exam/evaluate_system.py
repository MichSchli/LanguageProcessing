    ...
    for line in codecs.open(file_name, encoding='utf-8'):
        ...
        if line:
            ...
            if len(fields) not in [3,4]:
                print>>sys.stderr, "ERROS: File {} not in right format!".format(file_name)
                sys.exit()
...
# read in files to
reader_predictions = read_RE_file(args.prediction)
reader_gold = read_RE_file(args.gold)

predictions=list(reader_predictions)
gold=list(reader_gold)

num_pred=len(predictions) 
num_gold=len(gold) 
if num_pred!=num_gold:
    print>>sys.stderr, "ERROR: Number of sentences in gold and prediction file do not match: {} vs {}".format(num_gold,num_pred)
    exit()
print "Number of sentences: {}".format(num_gold)
...
for gold_instance, prediction_instance in zip(gold, predictions):
    ...
    if args.debug:
        print "predicted", prediction_instance
        print "gold", gold_instance
        print ""
...
print
print "{}\tPREC\tREC\tF1\t|\tTP\tFP\tFN".format("RELATION".ljust(20))
print '-' * 45
...
for relation in relations:
    ...
    print "%s\t%.4f\t%.4f\t%.4f\t|\t%.0f\t%.0f\t%.0f" % (relation.ljust(20), precision, recall, f1, true_positives[relation],false_positives[relation], false_negatives[relation])
...
print "MICRO-AVG.       \t%.4f\t%.4f\t%.4f\t|\t%.0f\t%.0f\t%.0f" % (precision, recall, f1, sum(true_positives.values()),sum(false_positives.values()),sum(false_negatives.values()))
