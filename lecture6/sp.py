...
class StructuredPerceptron(object):
    ...
    def get_features(self, word, tag, previous_tag):
        ...
        rest = word_lower[-3:3]
        vowels = [v for v in word if v in ['a','e','i','o','u','y']]
        characters = len(word)
        ...
        features = [
                    ...
                    'PREFIX+SUFFIX_%s_%s_%s' % (prefix, suffix, tag),
                    'EXCEPT_PREFIX_SUFFIX_%s_%s' % (rest, tag),
                    'VOWELS_TAG_%s_%s' % (vowels, tag),
                    'NO_OF_VOWELS_TAG_%s_%s' % (len(vowels), tag),
                    'CHARACTERS_TAG_%s_%s' % (characters, tag),
                    #########################
